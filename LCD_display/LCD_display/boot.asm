...
;
;  DESCRIPTION:
;  M8C Boot Code for CY8C22x45 microcontroller devices.
;
...
ENDIF
    ...
    org   50h                      ;SARADC Interrupt Vector
    ljmp	_SAR10_ISR
    reti

    org   54h                      ;CSD0 Interrupt Vector
    // call	void_handler
    reti

    org   58h                      ;CSD1 Interrupt Vector
    // call	void_handler
    reti

    org   5Ch                      ;RTC Interrupt Vector
    // call	void_handler
    reti
    
    org 50h                        ;SAR10 Interrupt Vector
    ljmp	_SAR10_ISR
    reti
...
ENDIF
    ...
    or reg[CPU_SCR1], CPU_SCR1_ECO_ALLOWED ; Allow ECO being enabled
...
ENDIF ; 5.0V Operation
    ...
    ;   -At 2.7 volt operation, the cpu rate must be at or below 3 MHz
...
IF (POWER_SETTING & POWER_SET_3V3)		; 3.3V Operation?
 ...
 ENDIF
ELSE
IF (POWER_SETTING & POWER_SET_2V7_12MHZ)	; 2.7V Operation with 12 MHz SLIMO?
   IF(CPU_CLOCK_JUST ^ OSC_CR0_CPU_24MHz)	; Not 12 MHz, do nothing. ("OSC_CR0_CPU_24MHz" setting is actually IMO/1)
   ELSE						; Otherwise, 12 MHz is selected... raise error
      ERROR_PSoC SEE COMMENTS			; Error - cannot use 12 MHz w/ 2.7V operation (Clock <= 3 MHz)
   ENDIF;
   IF(CPU_CLOCK_JUST ^ OSC_CR0_CPU_12MHz)	; Not 6 MHz, do nothing. ("OSC_CR0_CPU_12MHz" setting is actually IMO/2)
   ELSE						; Otherwise, 6 MHz is selected... raise error
      ERROR_PSoC SEE COMMENTS			; Error - Cannot use 6 MHz w/ 2.7V operation (Clock <= 3 MHz)
   ENDIF;CPU_12MHz
ELSE
IF (POWER_SETTING & POWER_SET_2V7_6MHZ)		; 2.7V Operation with 6 MHz SLIMO?
   IF(CPU_CLOCK_JUST ^ OSC_CR0_CPU_24MHz)	; Not 6 MHz, do nothing. ("OSC_CR0_CPU_24MHz" setting is actually IMO/1)
   ELSE						; Otherwise, 6 MHz is selected... raise error
	ERROR_PSoC SEE COMMENTS			; Error - cannot use 6 MHz w/ 2.7V operation (Clock <= 3 MHz)
   ENDIF
ENDIF ; 2.7v w/ 6 MHz
...
ENDIF ; 3.3V w/ 24 MHz
    ...
    ;  Technical Reference Manual #001-48461 for more information.
